...
Logo 专题不直接把表单字段拼成一句中文提示词，而是生成结构化提示词。每个风格方向生成一份独立提示词。
...
`briefSnapshot` 保存生成当时的品牌简报快照，避免用户修改项目后旧结果失去上下文。
...
1. 创建一个 Logo 轻项目。
2. 填写品牌简报和 Logo 方向。
3. 一次生成多个风格方向的 Logo 初稿。
4. 在结果区按方向比较方案。
5. 选择某个方案继续修改。
6. 查看白底、黑底、64px、32px 预览。
7. 把结果保存到对应 Logo 项目，而不是混入通用生成历史。
